...
curl -f -X POST https://api.github.com/repos/$USERNAME/$REPO/labels -H "Authorization: token $GH_TOKEN" -d '{"name":"version:major","description":"Gravierende Änderung mit Kompatibilitätsbruch","color":"ffffff"}'
curl -f -X POST https://api.github.com/repos/$USERNAME/$REPO/labels -H "Authorization: token $GH_TOKEN" -d '{"name":"version:minor","description":"Rückwärtskompatible Ergänzung / Erweiterung","color":"ffffff"}'
curl -f -X POST https://api.github.com/repos/$USERNAME/$REPO/labels -H "Authorization: token $GH_TOKEN" -d '{"name":"version:patch","description":"Geringfügige Korrektur ohne Risiko","color":"ffffff"}'
